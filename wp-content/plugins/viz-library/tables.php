...
function xls_to_array($x){
	$excelReader = PHPExcel_IOFactory::createReaderForFile($x);
	$excelObj = $excelReader->load($x);
	...
	foreach ($worksheet->getRowIterator() as $row) {				
		...
		foreach ($cellIterator as $cell) {		
			$v = trim( $cell->getCalculatedValue() );
			if($i==0){
				if($v != ""){
					$header[] = $v;
				}
...
if( $_POST["submit"] == "Upload"){
	if( $_POST["name"] !== ""  || $_POST["append"] !== "none" ){
		...
		if( array_key_exists($name, $tables) ){ 
			...
			#RETRIEVE INPUT CSVs OR XSL AND READ THEM TO ARRAYS
			$i=0; $z=count($_FILES["file"]["tmp_name"]);
			while($i<$z){	
				if($_FILES["file"]["type"][$i] == "text/csv" ){
					csv_to_array($_FILES["file"]["tmp_name"][$i]);
				}
				if( preg_match( "/.*\.xls\z|.*\.xlsx\z/", $_FILES["file"]["tmp_name"][$i]) ){
					xls_to_array($_FILES["file"]["tmp_name"][$i]);
				}
				$i++;
			}
